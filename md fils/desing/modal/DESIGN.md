...
name: Modal
description: |
  Modal's marketing site doesn't commit to one register the way most dark-mode
  developer tools do. It swaps between true black and a pale minty off-white
  section by section, and the one thread holding both halves together is a
  single saturated lime green (#7fee64) that never has to share the page with
  a second accent. Headlines run in Goga, a self-hosted variable font from a
  small Czech foundry, left at its natural letter-spacing rather than tracked
  tight for an editorial look. The feature panels aren't screenshots or code
  blocks; they're screen-captured video of the actual product, and the
  recurring cube motif (the logo, the Sandboxes icon, the security render)
  ties the illustration system back to what Modal actually sells: containers.
