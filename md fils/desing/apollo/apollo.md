version: alpha name: Apollo description: | Apollo's marketing site sits inside its own frame: a warm taupe margin (sand-40, #ccc9c6) surrounds a white content card on every page, so the whole site reads like a document laid on a desk rather than a surface that fills the browser edge to edge. Headlines run in Season Mix, a 2024 variable typeface from the Prague foundry Displaay that blends sans and serif construction in one file, tracked in tight at almost exactly one percent of its size at every breakpoint. The four product pillars, Outbound, Inbound, Data Enrichment, and Deal Execution, each carry their own named accent (sunbeam, iris, rust, ocean), so the palette functions as navigation as much as decoration. Real customer headshots and real screenshots carry the proof sections; the FAQ copy at the bottom of the page is the one place the site slips into the same formulaic, keyword-led pattern most B2B sites use.

colors: sand-10: "#f7f5f2" sand-20: "#edebe8" sand-30: "#e5e3e1" sand-40: "#ccc9c6" sand-50: "#94918e" sand-60: "#736f6c" sand-70: "#47423d" sand-80: "#1a1a1a" iris-05: "#f5f0ff" iris-10: "#c1b7ff" iris-50: "#9c73ff" iris-60: "#603fab" iris-100: "#3f3653" ocean-05: "#e9f2ff" ocean-10: "#c5ddff" ocean-50: "#3a6783" ocean-100: "#243031" rust-05: "#fff0fe" rust-10: "#ffbdf5" rust-50: "#ff8077" sunbeam-10: "#feffd9" sunbeam-20: "#fcffa8" sunbeam-50: "#f8ff2c" sunbeam-60: "#ebf212" sunbeam-70: "#c7cc43" text-header: "#000000" text-body: "#1a1a1a" text-placeholder: "#736f6c" text-negative: "#ff8077"

typography: hero: fontFamily: Season Mix fontSize: "44px / 56px / 64px / 72px / 80px across breakpoints" fontWeight: 550 lineHeight: "100% (leading-none)" letterSpacing: "-1% of size at every step" section-header: fontFamily: Season Mix fontSize: "36px / 40px / 48px / 56px across breakpoints" fontWeight: 550 lineHeight: "110%" letterSpacing: "roughly -0.9% of size" body: fontFamily: ABC Diatype fontSize: "14px / 16px at desktop-xl" fontWeight: 400 lineHeight: "130%" button: fontFamily: ABC Diatype fontSize: "14px / 16px at desktop-xl" fontWeight: 400 lineHeight: "130%" mono-label: fontFamily: Founders Grotesk Mono fontSize: 12px fontWeight: 400 lineHeight: "130%" letterSpacing: wide, uppercase

rounded: none: 0px xs: 2px sm: 4px lg: 8px xl: 12px 2xl: 16px full: 9999px

spacing: container-max: 1872px nav-height: "56px / 64px (tablet) / 72px (desktop-s and up)"

components: btn-primary: backgroundColor: "{colors.sunbeam-60}" textColor: "{colors.sand-80}" typography: "{typography.button}" rounded: "{rounded.lg}" height: 40px padding: 0 16px btn-secondary: backgroundColor: transparent textColor: "{colors.text-header}" border: "1px solid {colors.sand-40}" typography: "{typography.button}" rounded: "{rounded.lg}" height: 40px padding: 0 16px page-frame: backgroundColor: "{colors.sand-40}" rounded: "{rounded.none}" padding: 0 content-card: backgroundColor: "#ffffff" rounded: "{rounded.2xl}" padding: 0 pillar-tab: backgroundColor: "sunbeam-60 / iris-50 / rust-50 / ocean-50, one per pillar" textColor: "{colors.sand-80}" typography: "{typography.mono-label}" rounded: "{rounded.lg}" stat-card: backgroundColor: "#ffffff" textColor: "{colors.iris-100}" typography: "{typography.section-header}" rounded: "{rounded.xl}" padding: 24px testimonial-card: backgroundColor: "#ffffff" textColor: "{colors.text-body}" typography: "{typography.body}" rounded: "{rounded.xl}" padding: 24px faq-row: backgroundColor: transparent textColor: "{colors.text-header}" typography: "{typography.section-header}" border: "1px solid {colors.sand-30}" compliance-badge: backgroundColor: "{colors.sand-10}" textColor: "{colors.text-body}" typography: "{typography.mono-label}" rounded: "{rounded.lg}" nav-bar: backgroundColor: "{colors.sand-40}" typography: "{typography.button}" rounded: "{rounded.none}" height: "{spacing.nav-height}" ai-assistant-demo: backgroundColor: "gradient, amber to gold" rounded: "{rounded.2xl}" padding: 24px footer-ai-links: backgroundColor: transparent textColor: "{colors.text-body}" typography: "{typography.body}" rounded: "{rounded.none}"
...
Decide which of the four pillars a new section belongs to before choosing a color; the accent should follow the pillar, not the other way around.
Keep new headlines in Season Mix and new body or UI text in ABC Diatype. Don't blend them within the same text block.
Reference tokens directly ({colors.sunbeam-60}, {component.btn-primary}, {rounded.lg}) rather than restating raw values in prose.
Any new proof point (a stat, a quote, a case study) needs a real customer name attached before it ships; the page has no anonymous claims to model from.
If writing FAQ or help copy, model it on the pull quotes and stat cards, not on the existing FAQ section, which is the one part of the system worth deviating from.
Keep the sand page frame and white content card on any new marketing template; it's the single element that makes this site recognizable at a glance.
Known Gaps
